        ...
        <>
          ...
          >
            <div className="bg-brand-light w-full max-w-4xl rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row pointer-events-auto relative">
              ...
              {/* Image */}
              <div className="md:w-1/2 relative aspect-[4/5] md:aspect-auto h-[350px] md:h-auto">
                <Image
                  src={product.image}
                  alt={product.name}
                  fill
              ...
              <div className="md:w-1/2 p-10 md:p-14 flex flex-col justify-center">
                <div className="flex items-center gap-3 mb-4">
                  <span className="text-[10px] font-medium uppercase tracking-[0.3em] text-brand-dark/40">{product.category}</span>
                ...
                <p className="text-brand-dark/40 text-sm font-light leading-relaxed mb-10">
                  Handcrafted by artisan makers in Central Java using 100% organic cotton
                  and plant-based natural dyes. Each piece is unique, carrying the warmth of
                  the hands that made it.
                </p>

                <div className="space-y-6 mb-10">
                  <div className="flex items-center gap-4">
                    <span className="text-[10px] font-medium uppercase tracking-[0.15em] text-brand-dark/40">Size</span>
                    <div className="flex gap-2">
                      {['XS', 'S', 'M', 'L'].map(size => (
                        <button key={size} className="w-10 h-10 rounded-lg border border-brand-dark/10 flex items-center justify-center text-[11px] font-medium hover:border-brand-primary hover:text-brand-primary transition-all">
                          {size}
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="flex items-center gap-3 text-brand-secondary">
                    <ShieldCheck size={16} />
                    <span className="text-[10px] font-medium uppercase tracking-[0.15em]">In Stock — Ships within 3 days</span>
                  </div>
                </div>
