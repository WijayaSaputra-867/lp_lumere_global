...
const Collections = () => {
  return (
    <section id="collections" className="py-24 bg-white relative overflow-hidden text-zinc-900">
      <div className="container mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          ...
          >
            <h2 className="text-4xl md:text-5xl font-bold mb-4">Curated <span className="text-gradient">Collections</span></h2>
            <p className="text-zinc-600 max-w-md font-medium">Discover our carefully crafted pieces designed for every moment of your child's journey.</p>
          </motion.div>
          <motion.button
            initial={{ opacity: 0 }}
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {collections.map((item, index) => (
            ...
            >
              <div className={`relative aspect-[3/4] rounded-[32px] overflow-hidden mb-6 ${item.color}`}>
                <img 
                  src={item.image} 
                  alt={item.title}
              ...
              <p className="text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">{item.category}</p>
              <h3 className="text-2xl font-bold text-zinc-900 group-hover:text-brand-primary transition-colors">{item.title}</h3>
            </motion.div>
          ))}
        </div>
      </div>
